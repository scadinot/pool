...
[IMPORTANT]
Vous devez activer cette option uniquement au début de l'hivernage et la désactiver à la fin de l'hivernage pour revenir au cycle de filtration classique.
...
Dans cet onglet, vous devez définir les équipements permettant de `démarrer la filtration` et d' `arrêter la filtration`.
...
===== Onglet [Temps de filtration]

image::../images/config_temps_filtration.png["Onglet Filtration",align="Center"]
